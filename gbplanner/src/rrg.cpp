...
void Rrg::initializeAttributes() {
  ...
  local_graph_points_pub_ = nh_.advertise<geometry_msgs::PoseArray>(
      "gbplanner_node/local_graph_points", 10);
...
void Rrg::reset() {
  ...
  if (add_frontiers_to_global_graph_) {
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                  "Reset: Adding frontiers to global graph");
    add_frontiers_to_global_graph_ = false;
  ...
  if (MapManager::VoxelStatus::kFree != voxel_state) {
    switch (voxel_state) {
      ...
      case MapManager::VoxelStatus::kUnknown:
        ROS_INFO_COND(global_verbosity >= Verbosity::DEBUG,
                      "Current box contains Unknown voxels.");
        break;
    }
...
void Rrg::expandGraphEdges(std::shared_ptr<GraphManager> graph_manager,
  ...
  std::vector<Vertex*> nearest_vertices;
  if (!graph_manager->getNearestVertices(&(new_vertex->state), planning_params_.nearest_range, &nearest_vertices)) {
    rep.status = ExpandGraphStatus::kErrorKdTree;
    return;
  }
  ...
  for (int i = 0; i < nearest_vertices.size(); ++i) {
    ...
    direction << nearest_vertices[i]->state[0] - origin[0], nearest_vertices[i]->state[1] - origin[1],
        nearest_vertices[i]->state[2] - origin[2];
    double d_norm = direction.norm();
    if ((d_norm > planning_params_.edge_length_min) && (d_norm < planning_params_.edge_length_max)) {
      ...
      std::vector<Eigen::Vector3d> projected_edge;
      if (robot_params_.type == RobotType::kAerialRobot) {
        MapManager::VoxelStatus vs = map_manager_->getPathStatus(p_start, p_end, robot_box_size_, true);
        if (MapManager::VoxelStatus::kFree == vs) {
          admissible_edge = true;
        }
      } else if (robot_params_.type == RobotType::kGroundRobot) {
        ProjectedEdgeStatus es = getProjectedEdgeStatus(p_start, p_end, robot_box_size_, true, projected_edge, false);
        if (ProjectedEdgeStatus::kAdmissible == es) {
          admissible_edge = true;
        }
      }
...
void Rrg::expandGraph(std::shared_ptr<GraphManager> graph_manager,
  ...
  if (planning_params_.geofence_checking_enable && 
  (GeofenceManager::CoordinateStatus::kViolated == geofence_manager_->getPathStatus(
  ...
  std::vector<Eigen::Vector3d> projected_edge;
  if (robot_params_.type == RobotType::kAerialRobot) {
    MapManager::VoxelStatus vs = map_manager_->getPathStatus(start_pos, end_pos, robot_box_size_, true);
    if (MapManager::VoxelStatus::kFree == vs) {
      admissible_edge = true;
    }
  } else if (robot_params_.type == RobotType::kGroundRobot) {
    bool is_hanging = nearest_vertex->is_hanging || new_vertex.is_hanging;
    ProjectedEdgeStatus es = getProjectedEdgeStatus(start_pos, end_pos, robot_box_size_, true, projected_edge, is_hanging);
    if (ProjectedEdgeStatus::kAdmissible == es) {
      admissible_edge = true;
  ...
  if (admissible_edge) {
    ...
    if (planning_params_.rr_mode == RRModeType::kGraph) {
      std::vector<Vertex*> nearest_vertices;
      if (!graph_manager->getNearestVertices(&new_state, planning_params_.nearest_range, &nearest_vertices)) {
        rep.status = ExpandGraphStatus::kErrorKdTree;
        return;
      }
        ...
        if ((d_norm > planning_params_.nearest_range_min) &&
          ...
          if (geofence_pass) {
            ...
            if (robot_params_.type == RobotType::kAerialRobot) {
              ...
              ProjectedEdgeStatus es = getProjectedEdgeStatus(p_start, p_end, robot_box_size_, true, projected_edge, is_hanging);
              if (ProjectedEdgeStatus::kAdmissible == es) {
                admissible_edge = true;
                StateVec strt_st(projected_edge[0](0), projected_edge[0](1), projected_edge[0](2), 0.0);
                Vertex* strt_vert = new Vertex(projected_graph_->generateVertexID(), strt_st);
    ...
    stat_->num_edges_fail++;
    if (stat_->num_edges_fail < 500) {
      std::vector<double> vtmp = {start_pos[0], start_pos[1], start_pos[2], end_pos[0],   end_pos[1],   end_pos[2]};
      stat_->edges_fail.push_back(vtmp);
    }
    rep.status = ExpandGraphStatus::kErrorCollisionEdge;
    return;
...
void Rrg::expandGraphEdgesBlindly(std::shared_ptr<GraphManager> graph_manager, Vertex* new_vertex, double radius,
...
Rrg::GraphStatus Rrg::buildGraph() {
  ...
  ROS_INFO_COND(
      ...
      "Formed a graph with [%d] vertices and [%d] edges with [%d] loops",
  ...
  if (local_graph_->getNumVertices() > 1) {
    ...
    ROS_INFO_COND(global_verbosity >= Verbosity::DEBUG,
                  "Number of failed samples: [%d] vertices and [%d] edges",
...
Rrg::GraphStatus Rrg::buildGridGraph(StateVec state, Eigen::Vector3d robot_size,
  ...
  ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG,
                "Number of nodes [%d][%d][%d].", num_nodes[0], num_nodes[1],
                num_nodes[2]);
  ...
  ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG,
                "Number of free nodes [%d].", free_vertex_count);
  ...
  ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG, "Free edges: [%d]",
                free_edge_count);
...
Rrg::GraphStatus Rrg::evaluateGraph() {
  ...
  computeExplorationGain(planning_params_.leafs_only_for_volumetric_gain, planning_params_.cluster_vertices_for_gain);
        ...
        if (ind > 0 && robot_params_.type == RobotType::kGroundRobot) {
          ...
          if ((path[ind]->state(2) - path[ind - 1]->state(2)) < -map_manager_->getResolution()) {
            ...
            if (inclination > planning_params_.max_negative_inclination ||
                (std::atan2(std::abs(segment(2)), segment.head(2).norm()) > planning_params_.max_negative_inclination)) {
              path_gain = 0.0;
  ...
  if (planning_params_.auto_global_planner_enable) {
    if (!frontier_exists) {
      ++num_low_gain_iters_;
      ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG, "No frontier found in this round. Total rounds: %d", num_low_gain_iters_);
    } else {
      if (num_low_gain_iters_ > 0) --num_low_gain_iters_;
    }
    if (num_low_gain_iters_ >= 4) {
      ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG, "%d consecutinve low gain paths, triggering global planner.", num_low_gain_iters_);
      num_low_gain_iters_ = 0;
  ...
  visualization_->visualizeNegativePaths(inadmissible_negative_edges, local_graph_, local_graph_rep_);
...
bool Rrg::modifyPath(pcl::PointCloud<pcl::PointXYZ>* obstacle_pcl,
  ...
  if ((dist_min_sq == std::numeric_limits<double>::max()) ||
    ...
    ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG,
                  "[IMPRV] Path too close to obstacle");
    return false;
  }
...
void Rrg::addFrontiers(int best_vertex_id) {
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Global graph: %d vertices, %d edges.",
  ...
  if (update_global_frontiers) {
    ...
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                  "Have %d frontiers from global graph.",
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Get %d leaf vertices from newly local graph.",
                (int)leaf_vertices.size());
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Get %d frontiers from newly local graph.",
...
void Rrg::publishLocalGraphPoints() {
  std::pair<Graph::GraphType::vertex_iterator,
            Graph::GraphType::vertex_iterator>
      vi;

  std::shared_ptr<Graph> g = local_graph_->graph_;
  std::unordered_map<int, Vertex*>& v_map = local_graph_->vertices_map_;

  if (local_graph_->getNumVertices() == 0) return;
  if (local_graph_points_pub_.getNumSubscribers() < 1) return;
  ...
  local_graph_points_pub_.publish(pose_array);
}

void Rrg::publishGlobalGraphPoints() {
  std::pair<Graph::GraphType::vertex_iterator,
            Graph::GraphType::vertex_iterator>
      vi;
  ...
  if (global_graph_points_pub_.getNumSubscribers() < 1) return;
...
void Rrg::expandGlobalGraphFrontierAdditionTimerCallback(
  ...
  if (add_frontiers_to_global_graph_) {
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                  "Timer: Adding frontiers to global graph");
    add_frontiers_to_global_graph_ = false;
    addFrontiers(0);  // id given as 0 because it is not used
  }
}
...
void Rrg::semanticsCallback(
  ...
  if (MapManager::VoxelStatus::kFree !=
      map_manager_->getBoxStatus(sem + robot_params_.center_offset,
                                 robot_box_size_, true)) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[SEMANTICS]: Marker state not free.");
    return;
  }
  ...
  if (!global_graph_->getNearestVertex(new_state, &temp_nearest_vertex)) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[SEMANTICS]: No nearest vertex found.");
    return;
  }
  std::vector<Vertex*> nearest_vertices;
  ...
  if (!path_status) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[SEMANTICS]: Cannot connect to nearest node");
    return;
  } else {
...
ConnectStatus Rrg::findPathToConnect(
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Search a path from src [%f,%f,%f] to tgt [%f,%f,%f]",
                source[0], source[1], source[2], target[0], target[1],
                target[2]);
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::DEBUG,
                "Built a graph with %d vertices and %d edges.",
  ...
  if (reached_target) {
    ...
    if (voxel_state == MapManager::VoxelStatus::kFree) {
      ...
      if (rep.status == ExpandGraphStatus::kSuccess) {
        ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                      "Added target to the graph successfully.");
        num_vertices += rep.num_vertices_added;
        num_edges += rep.num_edges_added;
        added_target = true;
        ...
        ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                      "Cannot expand the graph to connect to the target.");
      }
    } else {
      ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                    "Target is not free, failed to add to the graph.");
    }
  } else {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "ConnectStatus::kErrorNoFeasiblePath");
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Get shortest path [%d] from %d path.", target_vertex->id,
...
void Rrg::computeExplorationGain(bool only_leaf_vertices, bool clustering) {
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "Num clusters: %d",
                num_clusters);
  stat_->compute_exp_gain_time = GET_ELAPSED_TIME(tim);
  t2 = std::chrono::high_resolution_clock::now();
  stat_chrono_->compute_exp_gain_time =
...
void Rrg::computeVolumetricGainRayModel(StateVec& state, VolumetricGain& vgain,
  ...
  for (int ind = 0; ind < planning_params_.exp_sensor_list.size(); ++ind) {
    ...
    // ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "Total number of
    // voxels queried: %d", (num_unknown_voxels + num_free_voxels +
    // num_occupied_voxels)); num_unknown_surf_voxels =
...
std::vector<geometry_msgs::Pose> Rrg::searchHomingPath(
  ...
  if (direction_norm <= kRadiusLimit) {
    ...
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[GlobalGraph] Try to add current state to the graph.");
    ExpandGraphReport rep;
    expandGraph(global_graph_, cur_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                    "[GlobalGraph] Added successfully.");
      ...
      ROS_WARN_COND(global_verbosity >= Verbosity::DEBUG,
                    "[GlobalGraph] Failed to find global path.");
    }
  }
  ...
  if (connect_state_to_graph) {
    if (!global_graph_->findShortestPaths(global_graph_rep_)) {
      ROS_ERROR_COND(global_verbosity >= Verbosity::ERROR,
                     "[GlobalGraph] Failed to find shortest path.");
      return ret_path;
    }
    ...
    if (homing_path_id.empty() || homing_path_id.back() != 0) {
      ROS_ERROR_COND(global_verbosity >= Verbosity::ERROR,
                     "[GlobalGraph] Could not find a path to home.");
      return ret_path;
    }
...
std::vector<geometry_msgs::Pose> Rrg::getGlobalPath(geometry_msgs::PoseStamped& waypoint) {
  ...
  if (!global_graph_->getNearestVertex(&wp, &wp_nearest_vertex)) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Cannot find any nearby vertex to reposition.");
    return ret_path;
  ...
  // from the second waypoint; the first waypoint keeps the same direction.
    if (planning_params_.yaw_tangent_correction) {
    bool is_similar = comparePathWithDirectionApprioximately(ret_path, tf::getYaw(ret_path[0].orientation));
      for (int i = 0; i < (ret_path.size() - 1); ++i) {
        Eigen::Vector3d vec;
        if ((!planning_params_.homing_backward) || (is_similar)) {
          vec << ret_path[i + 1].position.x - ret_path[i].position.x,
  ...
  if (planning_params_.path_safety_enhance_enable) {
    ...
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Compute an aternate path for homing in %f(s)", dmod_time);
...
std::vector<geometry_msgs::Pose> Rrg::getBestPath(std::string tgt_frame, int& status) {
  ...
  if (planning_params_.auto_homing_enable) {
    ...
    if (!homing_path.empty()) {
      ...
      ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "Time to home: %f; Time remaining: %f", time_to_home, time_remaining);
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
    "Best path:  size = %d, length = %f, time = %f", (int)ret.size(), traverse_length, traverse_time);
  ...
  while (cont_refine) {
    ...
    if (path_mod1.size() > 2) {
      for (int i = 0; i < (path_mod1.size() - 2); ++i) {
        ...
        // ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "Segment length
        // %f.", segment_len);
        if (segment_len < kSegmentLenMin) {
          if ((MapManager::VoxelStatus::kFree ==
            ...
            ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                          "Combine nodes to remove short segments.");
...
bool Rrg::addRefPathToGraph(const std::shared_ptr<GraphManager> graph_manager, const std::vector<Vertex*>& vertices) {
  ...
  for (int i = 0; i < vertex_list.size(); ++i) {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      switch (rep.status) {
        case ExpandGraphStatus::kErrorKdTree:
          ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                        "Can not add this vertex: kErrorKdTree.");
          break;
        case ExpandGraphStatus::kErrorCollisionEdge:
          ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                        "Can not add this vertex: kErrorCollisionEdge.");
          break;
        case ExpandGraphStatus::kErrorShortEdge:
          ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                        "Can not add this vertex: kErrorShortEdge.");
          break;
      }
    }
  }
...
bool Rrg::addRefPathToGraph(const std::shared_ptr<GraphManager> graph_manager, const std::vector<geometry_msgs::Pose>& path) {
...
void Rrg::setState(StateVec& state) {
  if (!odometry_ready) {
    ...
    // Reset the octomap
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Received the first odometry, reset the map");
...
void Rrg::timerCallback(const ros::TimerEvent& event) {
  ...
  if (!odometry_ready) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Planner is waiting for odometry");
    return;
  }
  ...
  if (planning_params_.auto_landing_enable && !landing_engaged_ &&
    ...
    if (time_budget_remaining <= 0.0) {
      if (!local_exploration_ongoing_) {
        ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                      "RAN OUT OF TIME BUDGET --> LANDING.");
        landing_engaged_ = true;
        std_msgs::Bool stop_msg;
        stop_msg.data = true;
        std_srvs::Empty empty_srv;
  ...
  if (cur_dir.norm() >= kOdoUpdateMinLength) {
    ...
    if (add_odometry_to_global_graph) {
      ...
      expandGraph(global_graph_, new_vertex, rep);
      // ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "From odometry,
      // added %d vertices and %d edges", rep.num_vertices_added,
      // rep.num_edges_added);
    }
...
std::vector<int> Rrg::performShortestPathsClustering(
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Cluster %d paths into %d clusters.", (int)vertices.size(),
  ...
  if (refinement_enable) {
    ...
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                  "Clustering with refinement %d paths into %d clusters.",
...
void Rrg::logMessage(const std::string& messageToLog) {
    // (Doncey Albin - 09/24/2024)
    std::ofstream log_file; // Init log stream (Doncey-Remove after testing)
    
    // Open file in append mode with a fixed log file path
    log_file.open(planning_params_.log_file_path, std::ios::app);
    
    if (log_file.is_open()) {
        // Log the passed message
        log_file << messageToLog << "\n";
        log_file.close(); // Close the file stream (Doncey-Remove after testing)
    } else {
...
std::vector<geometry_msgs::Pose> Rrg::runGlobalPlanner(int vertex_id, bool not_check_frontier, bool ignore_time) {
  ...
  if (global_graph_->getNumVertices() <= 1) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[GlobalGraph] Graph is empty, nothing to search.");
    return ret_path;
  }
  ...
  if (!ignore_time) {
    if (getTimeRemained() <= 0.0) {
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                    "[Global] RAN OUT OF TIME --> STOP HERE.");
      return ret_path;
    }
  }
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "Currently have %d frontiers in the global graph.",
                (int)global_frontiers.size());
  ...
  ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                "Trying to add new vertex from current position.");
  StateVec cur_state;
  cur_state << current_state_[0], current_state_[1], current_state_[2],
      current_state_[3];
  cur_state[2] -=
  ...
  if (!connected_to_graph) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Cannot add the state to the global graph.");
    return ret_path;
  }

  ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "Added current state to the graph. Start searching for the global path now.");
  ...
  if (vertex_id) {
    ...
    if (!isRemainingTimeSufficient(time_cost, time_spare)) {
      ...
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "[Global] Consider change to another ID or set ignore_time to True");
      return ret_path;
    }
    ...
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "Get %d feasible frontiers from global frontiers.", (int)feasible_global_frontiers.size());
    if (feasible_global_frontiers.size() <= 0) {
      ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "No feasible frontier exists --> Call HOMING instead if fully explored.");
      return ret_path;
    }
    ...
    for (int i = 0; i < feasible_global_frontiers.size(); ++i) {
      ...
      // const double kGDistancePenalty = 0.01; // Used as a param now
      double exp_gain = f->vol_gain.gain * exp(-planning_params_.kGDistancePenalty * current_to_frontier_distance);
    ...
    // Sort into descending order.
    std::sort(feasible_global_frontiers.begin(), feasible_global_frontiers.end(), [&frontier_exp_gain](const Vertex* a, const Vertex* b) {
                return frontier_exp_gain[a->id] > frontier_exp_gain[b->id];
              });
    // Print out
    // ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "List of potential
    // frontier in decreasing order of gain:"); for (int i = 0; i <
    // feasible_global_frontiers.size(); ++i) {
    //   ROS_INFO_COND(global_verbosity >= Verbosity::INFO, "ID [%d]: %d with
    //   gain %f", i, feasible_global_frontiers[i]->id,
  ...
  if (best_gain >= 0) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Found the best frontier to go is: %d", best_frontier->id);
  ...
  ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                "runGlobalPlanner costs: %f (s)", dtime);
...
void Rrg::clearUntraversableZones() {
  geofence_manager_.reset(new GeofenceManager());
  ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                "Clear all the traversable polygons.");
}
...
std::vector<geometry_msgs::Pose> Rrg::searchPathToPassGate() {
  ...
  if (!darpa_gate_params_.enable) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Not allow to run search to pass the gate");
    return ret_path;
  }
  ...
  if (darpa_gate_params_.load_from_darpa_frame) {
    ...
    try {
      ...
      ROS_ERROR_COND(global_verbosity >= Verbosity::ERROR, "%s", ex.what());
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                    "Could not look up TF for gate center.");
      return ret_path;
    }
  }
  ...
  if (ret_path.empty())
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "Could not find path to go through.");
  return ret_path;
}
...
bool RobotStateHistory::getNearestStates(const StateVec* state, double range, std::vector<StateVec*>* s_res) {
